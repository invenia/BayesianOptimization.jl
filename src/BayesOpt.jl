...
type BayesOpt
    ...
    """
    bounds::Vararg{AbstractArray{Real}}
    ...
    """
    function BayesOpt(f::Function, bounds::Vararg{AbstractArray{Real}})
        # Initialize our parameters matrix
        X = Array{Real}(length(bounds), 1)
...
function optimize!(opt::BayesOpt, iter=100, restarts=10)
    ...
    last_index = length(opt.y)

    # Allocate `iter` number of observation to both `X` and `y`
    opt.X = hcat(opt.X, Array{Real}(length(opt.bounds), iter))
    append!(opt.y, Array{Real}(iter))
...
- `BayesOpt`: a new `BayesOpt` instance.
"""
function optimize(f::Function, bounds::Varargs{AbstractArray{Real}}; iter=100, restarts=10)
...
function acquire_max(model, ymax, restarts, bounds)
    ...
    lower_bounds!(opt, [minimum(b) for b in bounds])
    upper_bounds!(opt, [maximum(b) for b in bounds]
    ...
    for i in 1:restarts
        ...
        x_try = [rand(b) for b in bounds]

        #Find the minimum of minus the acquisition function
        (minf,minx,ret) = optimize(opt, x_try)
